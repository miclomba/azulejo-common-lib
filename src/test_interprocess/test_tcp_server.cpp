...
TEST(TCPServer, Start)
{
	...
	EXPECT_CALL(server.GetAcceptor(), async_accept(_, _)).Times(2).WillRepeatedly(Invoke(&server.GetAcceptor(), &MockAcceptor::AsyncAcceptImpl));
...
TEST(TCPServer, HandleNewConnectionWithErrorCode)
{
	...
	TCPServer<MockHandler, MockAcceptor> server(context);
	EXPECT_CALL(server.GetAcceptor(), async_accept(_, _)).Times(1).WillRepeatedly(Invoke(&server.GetAcceptor(), &MockAcceptor::AsyncAcceptImplWithErrorParam));
